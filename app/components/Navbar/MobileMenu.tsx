...
const MobileMenu = ({
  ...
  return (
    <>
      <nav
        className={`${
          showMenu ? "translate-x-0 opacity-1" : "translate-x-full opacity-0"
        } relative w-full bg-black/20 backdrop-blur-lg mt-8 transition-all duration-300`}
      >
            ...
            return (
              <li className="relative" key={link.id}>
                <Link
                  href={link.link_url}
                  className={`${
                    link.link_url == pathname && "text-[var(--primary-color)]"
